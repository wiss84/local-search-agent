...
def _run(args: list[str]) -> tuple[int, str, str]:
    ...
    stderr_buf = StringIO()
    exit_code = 0
    with patch("sys.argv", ["local-search"] + args), \
...
class TestConfigShow:
    def test_show_exits_zero(self, tmp_path):
        with _patch_keys_path(tmp_path), _patch_models_path(tmp_path), _patch_settings_path(tmp_path):
...
class TestWorkspaceCommands:
    def test_workspace_create(self, tmp_path):
        ...
        with patch("local_search_agent.core.framework.SearchAgentFramework.create_workspace"):
            code, out, err = _run([
                "--db", db,
                "workspace", "create", "finance", str(tmp_path),
            ])
            assert code == 0
